...
class MethodPlaceNotationGenerator(PlaceNotationGenerator):
    def __init__(self, method_title: str, auto_start=True, logger=print):
        method_pn, le_pn, stage = self._load_method(method_title)
        if stage % 2:
            stage += 1
        super(MethodPlaceNotationGenerator, self).__init__(stage, method_pn, le_pn, auto_start=auto_start, logger=logger)
    ...
    def _load_method(method_title: str):
        ...
        if len(symblock) != 0:
            ...
            le = symblock[1].text
            return notation, le, stage
        elif len(block) != 0:
            notation = block[0].text
            return notation[:-1], notation[-1], stage
        else:
            return "", "", 4
